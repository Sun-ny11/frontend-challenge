...
class CatApi {
    ...
    async fetchCat({page = 0, userId, signal, limit = 15, order = "DESC"}:FetchCats): Promise<CatImage[]> {
    ...
    async likeCat(catId:string, userId:Nullable<string>): Promise<FavouriteCatResponse> {
